  ...
  <el-card  class="box-card" shadow="hover" :body-style="{ padding: '5px' }">
    <div class="clearfix">
      <div class="left">
        ...
        <span class="info" :title="info.reMainText" v-on:click="openFormEmit">{{ info.reMainText }}</span>
      </div>
      <el-tooltip v-if="info.tagFlag"  effect="dark" :content="info.reTag" placement="right">
        <el-button style="float: right; padding: 3px 0" type="text" v-on:click="openTags"
          >tag</el-button
        ></el-tooltip
      >
    </div>
...
export default {
  ...
  methods: {
    ...
    openTags () { // 点击tag打开tag视图
      router.push({name: 'tags', key: this.info.reKey})
    },
    openFormEmit () { // 点击卡片打开form
