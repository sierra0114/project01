    ...
    if (!window.indexedDB) {
      ...
      request.onupgradeneeded = (event) => { // 发现更新
        ...
        if (!db.objectStoreNames.contains('user')) { // 如果user存储库不存在
          ...
          objStore = db.createObjectStore('user', { keyPath: 'key', autoIncrement: true })// 对象仓库名字：‘user’，主键(key)
          console.log('·user创建存储库ing')
          objStore.createIndex('key', 'key', {unique: true})// 新建一个color索引// 新建一个user存储库
        }
      }
    }
  }
