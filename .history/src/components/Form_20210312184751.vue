...
export default {
  ...
  methods: {
    ...
    Delete() {
      ...
      request.onsuccess = (event) => {
        ...
        readRequest.onsuccess = () => {
          ...
          this.drawer = false;
          this.clearForm();
          mutations.setUpdateFlag(true); //发送更新数据的flag
          // alert("delete发送更新数据的flag");
        };
      };
    },
    // 清空表单中的数据
    ...
    getinfo() {
      ...
      let now;
      now = new Date();
      if (this.firstTime === true) {//  判断是否是第一次打开form
        // 如果是新建事项
      ...
      readRequest.onsuccess = () => {
        ...
        switch (instruct) {
          ...
          case "query":
            ...
            request.onerror = () => {
              alert("form查询所有info数据时失败");
            };
            break;
          default:
            alert("出现了未知错误，可能是由于instruct的值被改写");
            break;
        }
      };
    ...
    /// /////////////////////           改变颜色-开始           ///////////////////////////////
    showRemark() {
      this.show.remark = !this.show.remark;
    },
    showSummary() {
      this.show.summary = !this.show.summary;
    },
    showSubmission() {
      this.show.submission = !this.show.submission;
    },
    showRemind() {
      this.show.remind = !this.show.remind;
    },
    // --------------------------------------------------------------------------------
    /// /////////////////////           提醒-开始           ///////////////////////////////
    // ------------------------         改变颜色-结束          ----------------------
    open(info) {//已存在的info用form打开
      console.log(info);
      this.reInfo = info;
      this.firstTime = false;
