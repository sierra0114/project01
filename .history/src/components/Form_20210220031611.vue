    ...
    >
      <div id="color" :class="color">
        <el-row class="drawer-header">
          ...
          <el-col v-if="firstTime" :span="9" :offset="5" style="white-space: nowrap">
          ...
          <el-col v-else :span="9" :offset="5" style="white-space: nowrap">
            ...
            <el-button type="primary" @click="handleClose()" size="mini">保 存</el-button>
...
export default {
  ...
  methods: {
    ...
    validate () {
      ...
      if (pattern.test(this.form.textarea1)) { // 如果mainText输入框内容为空
    ...
    getinfo () {
      let info = {}
      let now = new Date()
      if (this.firstTime === true) { // 如果是新建事项
      ...
      // console.log(timeTo)
      info.time = this.form.timeValue.length === 0 ? [timeFrom, timeTo] : this.form.timeValue
      // 事项的起止时间
      ...
      readRequest.onsuccess = () => {
        ...
        switch (instruct) {
          ...
          case 'query':
            ...
            request.onerror = () => { console.log('查询所有info数据的事务 失败！') }
            break
          default:
    ...
    handleInputConfirm () {
      ...
      if (inputValue) { // 如果存在inputValue
        if (this.form.tag.dynamicTags.length === 0) { // 如果dynamicTags中不存在tag
          this.addTag()// 就将inputValue放入dynamicTags
        } else { // 如果dynamicTags中存在tag
          for (let i in this.form.tag.dynamicTags) { // 遍历每一个dynamicTags的tag
            let a = inputValue
            let b = this.form.tag.dynamicTags[i]
            console.log(a)
            console.log(b)
            if (a === b) { // 如果inputValue不与其中任何一个相等
    ...
    // --------------------------------------------------------------------------------
    /// /////////////////////           提醒-开始           ///////////////////////////////
    handleRemindChange () { // 处理 remind选择器 的 改变事件
