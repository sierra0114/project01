  ...
  methods: {
    ...
    querryTag(tag) {
      ...
      request.onsuccess = (event) => {
        ...
        index = objStore.index("tags");
        querryTransaction = index.get(tag);

        querryTransaction.onerror = () => {
          // console.log('读写事务 失败！')
        };
        querryTransaction.onsuccess = () => {
          // console.log('读写事务 成功！已经获取到了list数据')
          this.infolist = querryTransaction.result;
