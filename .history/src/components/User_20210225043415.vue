...
export default {
  ...
  methods: {
    ...
    deleteAccount() {
      this.$confirm("确认要删除账号吗？此操作无法撤销", "注意", {
        ...
        request.onsuccess = (event) => {
          ...
          readRequest = objStore.delete(this.user.id);

          readRequest.onerror = () => {
            alert("读取user存储库数据失败！-user");
          };
          readRequest.onsuccess = () => {
            Message.success("删除账号成功！");
            if (this.user.length > 0) {
              this.user = this.user[0].value;
              this.getTime();
              router.push({ name: "user", userId: this.id });
            } else {
